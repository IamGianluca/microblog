...
class TestCase():
    ...
    def test_follow(self):
        ...
        assert_equal(u1.unfollow(u2), None)

        u = u1.follow(u2)
        db.session.add(u)
        db.session.commit()

        assert_equal(u1.follow(u2), None)
        assert_equal(u1.is_following(u2), True)
        assert_equal(u1.followed.count(), 1)
        assert_equal(u1.followed.first().nickname, 'susan')
        assert_equal(u2.followers.count(), 1)
        assert_equal(u2.followers.first().nickname, 'john')

        u = u1.unfollow(u2)

        assert_not_equal(u, None)
    ...
    def test_follow_posts(self):
        ...
        assert_equal(len(f1), 3)
        assert_equal(len(f2), 2)
